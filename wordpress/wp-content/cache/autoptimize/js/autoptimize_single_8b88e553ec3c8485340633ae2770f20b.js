(function(){var PlayerIsLoaded=(typeof(localStorage.getItem('player_isload'))!="undefined"?((localStorage.getItem('player_isload')==1&&parseInt(localStorage.getItem('player_runningtime'))+10>Math.round(new Date().getTime()/1000))?1:0):0);var PlayerIsLoad=(typeof(PlayerIsLoad)!="undefined"?PlayerIsLoad:!PlayerIsLoaded);if(!PlayerIsLoad){console.log&&(console.log("%c%s","color: #81d742; background: black; font-size: 30px","WordPress浮窗音乐播放器\n取消加载：其他页面已加载"));return 0}
PlayerIsLoaded=1;localStorage.setItem('player_isload',1);setInterval(function(){localStorage.setItem('player_runningtime',Math.round(new Date().getTime()/1000))},5000)
window.onbeforeunload=function(){localStorage.setItem('player_isload',0)}
var FwmPlayerCode='<div id = "FwmPlayer" class = "show">'
+'  <div class = "player">'
...
+'  <div class = "block"></div>'
+'  <div class = "block"></div>'
+'</div>';$(document).ready(function(){$('body').append(FwmPlayerCode).fadeIn(5000);FwmPlayerCode=null;$.ajax({url:FwmURL+"/js/player.js?ver="+ver,dataType:"script",cache:1,async:1,success:function(data){window.console&&window.console.log&&(console.log("%c%s","color: #81d742; background: black; font-size: 30px","WordPress浮窗音乐播放器  \n作者:Eric               \n版本:"+ver+"加载成功！     \n主页:http://eric.cn.com/"))},error:function(error){}})})})()